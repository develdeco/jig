...
// with its stall signature, and the stalled remediation hint - the D8
// counterpart to TestStatusGoldenConstructed's plain building state.
func TestStatusGoldenStalled(t *testing.T) {
	...
	states := map[string]store.SliceState{
		"a": {State: "stalled", Attempts: 2, Reason: "stall", Signature: "sig-001"},
		"b": {State: "queued", Attempts: 0},
		"c": {State: "queued", Attempts: 0},
		"d": {State: "queued", Attempts: 0},
	}
